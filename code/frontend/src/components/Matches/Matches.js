...
function Matches() {
  ...
  // track button state per matched user: 'sending' | 'sent' | error string
  ...
  async function loadMatches() {
    ...
    try {
      ...
      // pre-populate which buttons should show "Request Sent"
  ...
  function getBarColor(percentage) {
    if (percentage >= 75) return '#2a7d4f';
    if (percentage >= 50) return '#E5751F';
    if (percentage >= 25) return '#861F41';
    return '#aaa';
  }
            ...
            return (
              <div key={index} className="match-card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  {status === 'sent' ? (
                    <span style={{
                      color: '#2a7d4f', fontWeight: 'bold', fontSize: '14px',
                      padding: '8px 12px', backgroundColor: '#d4edda', borderRadius: '5px'
                    }}>
                      Request Sent!
                    </span>
                  ) : status === 'sending' ? (
                ...
                <div style={{ marginTop: '12px', marginBottom: '6px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    <span style={{ fontSize: '13px', color: '#555', fontWeight: 'bold' }}>Match Score</span>
